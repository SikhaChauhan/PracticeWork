...
namespace Testing
{
    ...
    {
        private Mock<IAuthBL> _mockUserService;
        private AuthController _authController;

        [SetUp]
        public void Setup()
        {
            _mockUserService = new Mock<IAuthBL>();
        ...
        {
            // Arrange
            var userregisterDTO = new UserRegisterDTO { Email = "test@example.com", Password = "password", Name = "Test User" };
            var userEntity = new UserEntity { Id = 1, Email = userregisterDTO.Email, Name = userregisterDTO.Name };
            _mockUserService.Setup(x => x.Register(userregisterDTO)).ReturnsAsync(userEntity);
        ...
        public async Task Login_ShouldReturnOk_WhenCredentialsAreValid()
        {
            ...
            _mockUserService.Setup(x => x.Login(It.IsAny<UserLoginDTO>())).ReturnsAsync(token);
        ...
        {
            // Arrange
            var userDTO = new UserLoginDTO { Email = "test@example.com", Password = "wrongpassword" };
